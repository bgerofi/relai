...
checksummed copy, so it is installed deterministically rather than generated by
the model. It persists across sessions and is kept outside your repositories
(in `~/.relai/`) so it never shows up in `git status`.
...
`run`, and `info`. Edits are guarded: `--dry-run` returns a unified diff instead
of writing, `--expect-count` refuses a replace whose match count differs, every
write auto-saves a `.relai.bak`, and a `.py` edit that breaks syntax returns
`exit=4`. Every content payload is passed as base64 and every result is
...
modified. Because that command relies only on the target's own `python3`, it
works unchanged whether the foreground shell is local or a remote host reached
over `ssh`, and the helper itself is written to run on any Python 3.2+.
...
In short: tools are *how the agent acts*; the helper is a bundled script relai
installs on the target so the agent can act more precisely. Helpers are a
convenience, not a requirement — if they are missing, run `/init_helpers` to
install them, or the agent can fall back to plain shell commands.